    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-slate-50 rounded-2xl p-5 text-center">
        <div className="flex items-center justify-center gap-3">
          <span className={cn('font-bold text-slate-800', isDutchSource ? 'text-3xl' : 'text-2xl')}>
            {sourceText}
          </span>
      ...
      <div className="space-y-2">
        <input
          ...
          className={cn(
            ...
            !isChecked && 'bg-white border-slate-200 text-slate-800 focus:border-primary-900',
            isChecked && isCorrect && 'bg-emerald-50 border-emerald-500 text-emerald-800',
            isChecked && isCorrect === false && 'bg-red-50 border-red-400 text-red-700',
          )}
        />
        {isChecked && isCorrect === false && (
          <div className="flex items-center gap-2 text-sm text-slate-600 bg-slate-50 rounded-xl px-4 py-2">
            <span>
              Correct answer:{' '}
              <span className="font-bold text-slate-800">{exercise.correctAnswer}</span>
            </span>
